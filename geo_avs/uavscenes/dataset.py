...
def load_lidar_txt(path: str | Path) -> torch.Tensor:
    return torch.as_tensor(np.loadtxt(path, dtype=np.float32), dtype=torch.float32)


def load_label_rgb_txt(path: str | Path) -> torch.Tensor:
    return torch.as_tensor(np.loadtxt(path, dtype=np.uint8), dtype=torch.uint8)
...
def sample_dict(xyz_path: str | Path, rgb_label_path: str | Path | None = None) -> Dict:
    out = {"xyz": load_lidar_txt(xyz_path)}
    if rgb_label_path is not None:
        out["label_rgb"] = load_label_rgb_txt(rgb_label_path)
    return out
